...
class OrderLineInlineFormSet(
    inlineformset_factory(Order, OrderLine,
        ...
        widgets={
        'qty': NumInput(attrs={'min': '0', 'step': 'any', 'class': 'form-control col-centered line_calc line_qty', 'pattern': '[0-9*]', 'name': 'spin'}),
...
class BaseOrderStatusFormSetTest(BaseModelFormSet):
     def has_changed(self):
         ...
         isChanged = super(BaseOrderStatusFormSetTest, self).cleaned_data.get('is_changed')
