...
def sign_outs_remainder(request):
    ...
    Sum('orderline__qty')).values())[0]
    currentVials = list(orders.prefetch_related('orderline_set').filter(orderline__inventory=1263).filter(date_created__range=[lastbill, today]).filter(status__icontains='Complete').aggregate(
    Sum('orderline__qty')).values())[0]
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if 'exportCSV' in request.POST:
                ...
                e_reports = reports.values_list('id','requester__username', 'submitter__username', 'requester__department__number', 'date_created', 
...
def searchtest(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # paginator = Paginator(reports, 300)
            # try:
