...
def sign_outs_remainder(request):
    ...
    currentBottles = list(orders.prefetch_related('orderline_set').filter(orderline__inventory=1245).filter(date_created__range=[today, nextbill]).exclude(status__icontains='Canceled').aggregate(
    Sum('orderline__qty')).values())[0]
    currentVials = list(orders.prefetch_related('orderline_set').filter(orderline__inventory=1263).filter(date_created__range=[today, nextbill]).exclude(status__icontains='Canceled').aggregate(
    Sum('orderline__qty')).values())[0]
