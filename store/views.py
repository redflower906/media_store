...
def create_order(request, copy_id=None):
    ...
    if request.method == "POST":
        ...
        order_form = OrderForm(request.user, request.POST, request.FILES, prefix='order', instance=order, initial={'submitter':request.user, 'requester': request.user})
        ...
        if copy_id:
            ...
            order_form = OrderForm(request.user, prefix='order', instance=order, initial={'submitter': request.user.userprofile.name(), 'requester': request.user})
            orderlineformset = OrderLineInlineFormSet(
