...
def create_signout(request):
    ...
    return render(request, 'store/SO_create_test.html', {
        'order_form' : order_form,
        'formset': orderlineformset,
        'media_types': MEDIA_CHOICES,
        'loc': loc,
...
def sign_outs_remainder(request):
    ...
    twobills = datetime.strptime(str(today.year) + '-' + str(today.month) + '-' + '25','%Y-%m-%d' ).date() + relativedelta.relativedelta(months=-2)

    if today >= nextbill:
        nextbill = datetime.strptime(str(today.year) + '-' + str(today.month) + '-' + '25','%Y-%m-%d' ).date() + relativedelta.relativedelta(months=1)
        lastbill = datetime.strptime(str(today.year) + '-' + str(today.month) + '-' + '25','%Y-%m-%d' ).date()
        twobills = datetime.strptime(str(today.year) + '-' + str(today.month) + '-' + '25','%Y-%m-%d' ).date() + relativedelta.relativedelta(months=-1)

    currentBottles = list(orders.prefetch_related('orderline_set').filter(orderline__inventory=1245).filter(date_created__range=[twobills, today]).filter(status__icontains='Complete').aggregate(
    Sum('orderline__qty')).values())[0]
    currentVials = list(orders.prefetch_related('orderline_set').filter(orderline__inventory=1263).filter(date_created__range=[twobills, today]).filter(status__icontains='Complete').aggregate(
    Sum('orderline__qty')).values())[0]
...
def ajax(request):
    ...
    data = {'r_id': user.id, 'd_id':department.id, 'p_id':project, 'up_id':user.user_profile.id, 'd_cc':department.number, 'd_name':department.department_name}
