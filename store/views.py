...
def view_order(request):
    ...
    recur_queryset = orders.filter(is_recurring=True).filter(status__icontains='Submitted').filter(status__icontains='In-Progress').filter(status__icontains='Problem').exclude(date_recurring_stop__lte = today).prefetch_related('orderline_set').exclude(orderline__inventory__id='686')
